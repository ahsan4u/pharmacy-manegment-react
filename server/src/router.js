...
router.route('/medicines').get(async (req, res) => {
    const { sort, searchType = 'name', search, page } = req.query;

    let sortObj = {};
    if (sort === 'name') sortObj = { name: 1 };
    if (sort === 'expiry') sortObj = { exp: 1 };
    if (sort === 'qty-low') sortObj = { quantity: 1 };
    if (sort === 'qty-high') sortObj = { quantity: -1 };
    if (sort === 'price-low') sortObj = { mrp: 1 };
    if (sort === 'price-high') sortObj = { mrp: -1 };

    try {
        const medicines = await Medicines.find({ [searchType]: new RegExp(search, 'i') }).sort(sortObj).skip((page - 1) * limit).limit(limit);
        res.status(200).send(medicines);
    } catch (error) {
        console.log(error.message);
        res.status(500).send({ message: error.message });
